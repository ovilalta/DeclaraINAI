...
{
    public partial class Administradores : Pagina
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                // Ruta del archivo
                string rutaArchivo = Server.MapPath("~/Formas/Administrador/Administradores.txt");
        ...
        private List<DatoArchivo> LeerArchivo(string ruta)
        {
            ...
            if (File.Exists(ruta))
            {
                ...
                {
                    lineas.Add(new DatoArchivo { Rfc = linea.Trim() });
                }
            }
            else
            {
        ...
        protected void btnAgregar_Click(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(nuevoDato))
            {
                if (nuevoDato.Length==13)
                {
                    ...
                    File.AppendAllText(rutaArchivo, nuevoDato.ToUpper().Trim() + Environment.NewLine);
                    ...
                    BitacoraAdmin.RegistraBitacoraAdmin(_oUsuario.VID_NOMBRE + _oUsuario.VID_FECHA + _oUsuario.VID_HOMOCLAVE
                        , "Agregar admin", "Se agregó como admin a: " + nuevoDato.ToUpper());
        ...
        protected void gvExcelData_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "Eliminar")
            {
                ...
                if (indice >= 0 && indice < lineas.Count)
                {
                    ...
                    //Registra la búsqueda en bitácora
                    BitacoraAdmin.RegistraBitacoraAdmin(_oUsuario.VID_NOMBRE + _oUsuario.VID_FECHA + _oUsuario.VID_HOMOCLAVE
                        , "Eliminar admin", "Se eliminó como admin a: " + valorEliminado.ToUpper().Trim());
                    // Recargar los datos en el GridView
                    //CargarDatos();
                }
                CargarDatos();
