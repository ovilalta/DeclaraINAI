    ...
    public partial class Envio : Pagina, IDeclaracionInicial
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                SiNo.Checked = oDeclaracion.L_AUTORIZA_PUBLICAR.Value;
            }
        ...
        private void enviar()
        {
            try
            {
                if (_oDeclaracion.DECLARACION_APARTADOs.Where(p => !(new Int32[] { 0, 15, 17 }).Contains(p.NID_APARTADO) && p.L_ESTADO == false).Any())
                {
        ...
        public void Siguiente()
        {
            ...
            if (oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 15).First().L_ESTADO.Value)
            {
                QstBoxEnviar.AskWarning(
                                         "Declaro bajo protesta de decir verdad que toda la información manifestada en la presente declaración es verdadera y corresponde a la situación de mi patrimonio a la fecha de la conclusión del encargo, considerando en su caso, también los correspondientes a mi cónyuge, concubina o concubinario y sus dependientes económicos directos, y que no omití información alguna, en cumplimiento a lo establecido en los artículos 32, 33,fracción III, 34, 35 y 39 de la Ley General de Responsabilidades Administrativas."
                                        + "¿Desea enviar la declaración? <br />Debe tomar en cuenta que una vez enviada la declaración no podrá hacer modificaciones"
                                        );
            }
            else
            {
        ...
        protected void btnPrevisualizar_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                if (!oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 15).First().L_ESTADO.Value)
                {
                    ...
                    _Conclusion.Imprimir(oDeclaracion,true);
                }
                else
                {
                    cambiarBoton();
