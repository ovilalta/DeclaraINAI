...
namespace DeclaraINE.Formas.DeclaracionConclusion
{
    public partial class Adeudos : Pagina, IDeclaracionInicial
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (oDeclaracion.ALTA.ALTA_ADEUDOs.Count == 0 && !oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 27).First().L_ESTADO.Value)
            ...
            if (oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 27).First().L_ESTADO.Value)
