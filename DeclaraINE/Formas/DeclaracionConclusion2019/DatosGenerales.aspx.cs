...
namespace DeclaraINE.Formas.DeclaracionConclusion
{
    public partial class DatosGenerales : Pagina, IDeclaracionInicial
    {
        ...
        protected void Page_Init(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                txtF_POSESION_C.StartDate = new DateTime(2017, 1, 1);
        ...
        public void Anterior()
        {
            switch (SubSeccionActiva)
            {
                ...
                case SubSecciones.DomicilioLaboral:
                    ltrSubTitulo.Text = "Datos Generales – Cargo que Concluye";
                    pnlDatosPersonales.Visible = false;
                    pnlDomicilioParticular.Visible = false;
                    ...
                    case SubSecciones.Cargo:
                        if (lBanderaActualizaAnterior)
                        {
                            try
                            {
                                ...
                                oDeclaracion.DECLARACION_CARGO.NID_PUESTO = Convert.ToInt32(cmbVID_CLAVEPUESTO.SelectedValue);
                                oDeclaracion.DECLARACION_CARGO.V_DENOMINACION = txt_DENOMINACION_CARGO.Text;
                            ...
                            catch (Exception ex)
                            {
                                if (ex is Declara_V2.Exceptions.RowNotFoundException)
                                {
                                    ...
                                    oDeclaracion.DECLARACION_CARGO = new blld_DECLARACION_CARGO(oDeclaracion.VID_NOMBRE
                                                                                               ...
                                                                                               , txtF_POSESION.Date(esMX)
                                                                                               , txtF_INGRESO.Date(esMX)
                        ...
                        if (lBanderaPasa)
                        {
                            ...
                            try
                            {
                                ...
                                txtV_DOMICILIO_LABORAL.Text = oDeclaracion.DECLARACION_DOM_LABORAL.V_DOMICILIO;
                    ...
                    case SubSecciones.DomicilioLaboral:
                        ...
                        if (lBanderaActualizaAnterior)
                        {
                            try
                            {
                                ...
                                oDeclaracion.DECLARACION_DOM_LABORAL.V_DOMICILIO = txtV_DOMICILIO_LABORAL.Text;
                        ...
                        break;
                    case SubSecciones.DependienteEconomicos:
                        if (oDeclaracion.DECLARACION_DEPENDIENTESs.Where(p => p.F_NACIMIENTO.Equals(new DateTime(1900, 1, 1))).Any())
                            throw new CustomException("Debe actualizar la fecha de nacimiento de todos los dependientes");
                            if (oDeclaracion.DECLARACION_DEPENDIENTESs.Count == 0 && !oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 6).First().L_ESTADO.Value)
                        { }
                        else
                        {
        ...
        {
            blld_DECLARACION oDeclaracion = _oDeclaracion;
            ((_Conclusion)Master).links_DatosGenerales(ref oDeclaracion);
            switch (SubSeccionActiva)
            {
                case SubSecciones.DatosPersonales:
        ...
        protected void btnGuardaDependiente_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (ctrlDependiente1.Accion == ctrlDependiente.Acciones.Actualiza)
                {
                    ...
                    oDeclaracion.DECLARACION_DEPENDIENTESs[ctrlDependiente1.Selected_id].E_DOMICILIO = ctrlDependiente1.V_DOMICILIO;
                ...
                mdlDependiente.Hide();
               if (!oDeclaracion.DECLARACION_DEPENDIENTESs.Where(p => p.F_NACIMIENTO.Equals(new DateTime(1900,1,1))).Any()) marcaApartado(ref oDeclaracion, 6);
                _oDeclaracion = oDeclaracion;
        ...
        {
            QstEjercicio.AskDanger("Su declaración corresponde al ejercicio: <b>" + Convert.ToDateTime(Anio).Year.ToString() + "</b>. Si es correcto continué en caso contrario corrija la fecha de conclusión del encargo.</b><br><br><br> <h3><p style='color:#FF0000';> ¿Esta de Acuerdo? </p></h3>");
