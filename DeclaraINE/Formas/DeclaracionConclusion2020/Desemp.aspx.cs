    ...
    public partial class Desemp : Pagina, IDeclaracionInicial
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                grdPreguntas.DataSource = oDeclaracion.MODIFICACION.MODIFICACION_INGRESOSs.Where(p => p.VID_NOMBRE == oDeclaracion.VID_NOMBRE && p.VID_FECHA == oDeclaracion.VID_FECHA && p.VID_HOMOCLAVE == oDeclaracion.VID_HOMOCLAVE && p.NID_DECLARACION == oDeclaracion.NID_DECLARACION && p.NID_INGRESO > 99).ToList();
            ...
            ltrSubTitulo.Text = "¿Te desempeñaste como servidor público en el año inmediato anterior?";
        ...
        internal static string TomaDesicion(int NID_INGRESO)
        {
            ...
            try
            {
                var query = ((from p in dbInt.CAT_SECCION_INGRESO
                              where p.NID_RUBRO == NID_INGRESO && p.NID_SECCION == 2
        ...
        private void SumaDatos()
        {
            ...
            foreach (GridViewRow row in grdPreguntas.Rows)
            {
                ...
                if (Analisis.Split('|')[0] != "F" && Analisis.Split('|')[1] == "102")
                {
                ...
                else if (Analisis.Split('|')[0] != "F" && Analisis.Split('|')[1] == "114")
                {
                    V_COMPLEMENTO = ((TextBox)row.FindControl("txtMonto")).Text.Replace("$", "").Replace(",", "");
                    if (V_COMPLEMENTO.Length == 0)
                        V_COMPLEMENTO = "0";
                ...
                else if (Analisis.Split('|')[0] != "F" && Analisis.Split('|')[1] == "116")
                {
                    V_COMPLEMENTO = ((TextBox)row.FindControl("txtMonto")).Text.Replace("$", "").Replace(",", "");
                    if (V_COMPLEMENTO.Length == 0)
                        V_COMPLEMENTO = "0";
            ...
            foreach (GridViewRow row in grdPreguntas.Rows)
            {
                if (((Label)row.FindControl("NID_INGRESO")).Text == "102")
                {
                ...
                if (((Label)row.FindControl("NID_INGRESO")).Text == "114")
                {
                    ((TextBox)row.FindControl("txtMonto")).Text = Convert.ToString(Suma2 + Suma12);
                    ((TextBox)row.FindControl("txtMonto")).ToolTip = DescribeCifras(((TextBox)row.FindControl("txtMonto")).Text);
                }
                if (((Label)row.FindControl("NID_INGRESO")).Text == "116")
                {
        ...
        private void Limpia()
        {
            this.txtF_FINAL.Text = System.DateTime.Today.ToShortDateString();
            this.txtF_INGRESO.Text = System.DateTime.Today.ToShortDateString();
