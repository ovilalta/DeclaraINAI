...
namespace DeclaraINAI.Formas.DeclaracionModificacion
{
    public partial class ExperienciaLaboral : Pagina, IDeclaracionInicial
    {
        ...
        public void Siguiente()
        {
            ...
            {
                blld_DECLARACION oDeclaracion = _oDeclaracion;
                int nExp = oDeclaracion.DECLARACION_EXPERIENCIA_LABORALs.Count();
                if (!oDeclaracion.DECLARACION_APARTADOs.Where(p => p.NID_APARTADO == 17).First().L_ESTADO.Value)
                {
                ...
                // INCORPORASTE X DE 5 EMPLEOS ANTERIORES SOLICITADOS,  ¿ESTAS SEGURO DE CONTINUAR?
                //QstSiguiente.Ask("Incorporaste " + nExp + " de 5 empleos anteriores solicitados, ¿estás seguro de continuar?");
            }
            catch (Exception ex)
            {
            }
        ...
        public void NuevaExperienciaLaboral()
        {
            ...
            //{
                ...
                oDeclaracion.Add_DECLARACION_EXPERIENCIA_LABORALs(
                   ...
                   Convert.ToDateTime( ctrlExperienciaLaboral.F_INGRESO.ToString(strFormatoFecha)),
                    Convert.ToDateTime(ctrlExperienciaLaboral.F_EGRESO.ToString(strFormatoFecha)),
        ...
        protected void QstBox_Yes(object Sender, EventArgs e)
        {
            try
            {
                ...
                o = oDeclaracion.DECLARACION_EXPERIENCIA_LABORALs[lDeside];
                oDeclaracion.DECLARACION_EXPERIENCIA_LABORALs.RemoveAt(lDeside);
                grd.Controls.Remove(grd.FindControl(String.Concat("grd", lDeside)));
                _oDeclaracion = oDeclaracion;
                AlertaSuperior.ShowSuccess("Se eliminó correctamente la experiencia laboral");
            }
            catch (Exception ex)
