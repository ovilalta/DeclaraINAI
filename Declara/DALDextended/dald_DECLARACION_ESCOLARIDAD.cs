...
namespace Declara_V2.DALD
{
    internal partial class dald_DECLARACION_ESCOLARIDAD : dal_DECLARACION_ESCOLARIDAD
    {
        ...
        #region *** Metodos ***
        internal static int nNuevo_NID_ESCOLARIDAD(String VID_NOMBRE, String VID_FECHA, String VID_HOMOCLAVE, Int32 NID_DECLARACION)
        {
            MODELDeclara_V2.cnxDeclara dbInt = new MODELDeclara_V2.cnxDeclara();
            try
            {
                var query = ((from   p in dbInt.DECLARACION_ESCOLARIDAD
                              where  p.VID_NOMBRE == VID_NOMBRE && 
                                    p.VID_FECHA == VID_FECHA && 
                                    p.VID_HOMOCLAVE == VID_HOMOCLAVE && 
                                    p.NID_DECLARACION == NID_DECLARACION
                              select p.NID_ESCOLARIDAD).Max());
                return query + 1;
            }
            catch
            {
